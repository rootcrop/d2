...
def plot_line_stock(stock_data):
    ...
    """
    sns.set(style="darkgrid")       # whitegrid, darkgrid, whitegrid, dark, white, and ticks
...
def fetch_coin_price_data(coin_name, days=30):
    """
    функция извлекает исторические данные о цене монеты (coin_name) из API CoinGecko, за указанное количество дней
    и записывает полученные данные в файл coingecko_api_response_{coin_name}.json
    ...
    data = response.json()          # получаем dict/словарь вида : {'prices': [[1731888000000, 89841.47194135105], ...
    print(type(data))

    try:                            # пытаемся взять prices из словаря ответа coingecko
        prices = data['prices']     # получаем список из даты и цены [[1731888000000, 89841.47194135105], [1731974400000, 90534.62459650292] ...
    except Exception as err:
        print('error on get price of coin name: ' + coin_name)#, ' ', err)     # убираем )# если нужно вывести ошибку
        exit()

    with open(f'coingecko_api_response_{coin_name}.json', 'w', encoding="utf-8") as f:  # записываем ответ в файл
        f.write(str(response.json()))

    df = pd.DataFrame(prices, columns=['timestamp', 'price'])  # конвертируем словарь в дата_фрэйм
    '''   
      timestamp price
    0 1731888000000 89841.471941
    1 1731974400000 90534.624597
    '''
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')    # переводим timestamp в дату
    '''
      timestamp  price
    0 2024-11-18 00:00:00 89841.471941
    1 2024-11-19 00:00:00 90534.624597
    '''
    return df


def lineplot_bitcoin(coin_df, coin, days):  #  функция строит исторический график цены криптовалюты
    plt.figure(figsize=(14, 7))
    sns.lineplot(data=coin_df, x='timestamp', y='price', marker='o')    # создаем линейный график с помощью sns.lineplot.
...
# вызываем функции построения графиков для визуализации данных.
